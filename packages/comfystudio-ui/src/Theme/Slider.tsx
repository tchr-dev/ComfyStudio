...
export const Slider = ({
  ...
  const onBeforeChange = useCallback(() => setMoving(true), [setMoving]);

  const onAfterChange = useCallback(() => setMoving(false), [setMoving]);
  ...
  const renderTrack = useCallback(
    (
      ...
      const style = {
        ...(trackProps.style as React.CSSProperties),
      ...
      return (
        <div
          {...(trackProps as React.HTMLAttributes<HTMLDivElement>)}
          style={style}
          className={classes(
            "h-[2px] rounded-full",
  ...
  const renderThumb = useCallback(
    (thumbProps: Record<string, unknown>) => (
      <div
        {...(thumbProps as React.HTMLAttributes<HTMLDivElement>)}
        style={{ ...(thumbProps.style as React.CSSProperties), zIndex: 3 }}
  ...
  const renderMark = useCallback(
    (markProps: Record<string, unknown>) => (
      <span {...(markProps as React.HTMLAttributes<HTMLSpanElement>)} />
    ),
    []
  );
  ...
  return (
    ...
    >
      ...
      {/* @ts-expect-error ReactSlider types incompatible with React 18 ElementClass */}
      <ReactSlider
