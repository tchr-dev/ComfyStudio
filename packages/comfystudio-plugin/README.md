...
In order to make ComfyStudio easier to extend, we've ripped out the "back-end" into a plugin system.

This means you can implement an entirely different inference stack, ComfyStudio doesn't care if it's local or a hosted API.

## ⭐️ First-Party Plugins

There are currently three first-party plugins which are maintained in this repository:

- [`comfystudio-plugin-stability`](../comfystudio-plugin-stability/src/index.ts) – The default plugin which uses [Stability's API](https://platform.stability.ai) for inference.

  ```bash
  yarn dev
  ```

- [`comfystudio-plugin-webui`](../comfystudio-plugin-webui/README.md) – This plugin uses [`stable-diffusion-webui`](https://github.com/AUTOMATIC1111/stable-diffusion-webui) for inference.
...
**If you want to play around with an example plugin which has nice developer tooling, check out the [example plugin](../comfystudio-plugin-example/src/index.ts).**

You can load the UI using example plugin instead of the default [Stability plugin](../comfystudio-plugin-stability/src/index.ts) by running...

```bash
yarn dev:use-example-plugin
```

Any changes you make within the [example plugin](../comfystudio-plugin-example/src/index.ts) will be hot-reloaded into ComfyStudio.
...
ComfyStudio calls these functions for core functionality, such as `createStableDiffusionImages`, `getStableDiffusionStyles`, `getStableDiffusionDefaultInput`, etc.
...
Here's what's on our mind when it comes to the future of ComfyStudio plugins...

- **🧠 More Inference Stacks**

  The default [Stability plugin](../comfystudio-plugin-stability/src/index.ts) enables inference relying on [Stability's API](../comfystudio-plugin-stability/src/index.ts), but we're hoping to expand support for local inference.
  ...
  [`comfystudio-plugin-webui`](../comfystudio-plugin-webui/src/index.ts)

  [`comfystudio-plugin-webgpu`](../comfystudio-plugin-webgpu/src/index.ts)
