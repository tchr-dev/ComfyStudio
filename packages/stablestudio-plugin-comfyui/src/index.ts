...
const manifest: StableStudio.PluginManifest = {
  name: "ComfyUI",
  author: "StableStudio Community",
  link: "https://github.com/comfyanonymous/ComfyUI",
  icon: `${window.location.origin}/DummyImage.png`,
  version: "0.1.0",
  license: "MIT",
  description: `This plugin connects StableStudio to a local [ComfyUI](https://github.com/comfyanonymous/ComfyUI) installation for image generation.
  ...
  return {
    ...
    createStableDiffusionImages: async (options) => {
      ...
      if (input.initialImage?.blob) {
        ...
        inputImageName = `stablestudio_input_${timestamp}.png`;
        await client.uploadImage(input.initialImage.blob, inputImageName);

        if (input.maskImage?.blob) {
          maskImageName = `stablestudio_mask_${timestamp}.png`;
